...
def todo(ctx):
    ...
    #Open todo.txt - first line contains latest ID, reset contain tasks and IDs
    with open('./todo.txt') as f:
        content = f.readlines()
    #Transfer data from todo.txt to the context
    ctx.object['LATEST'] = int(content[:1][0])
    ctx.object['TASKS'] = {en.split('```')[0]:en.split('```')[1][:-1] for en in content[1:]}
...
def tasks(ctx):
    ...
    if ctx.obj['TASKS']:
        ...
        for i, taks in ctx.obj['TASKS'].items():
...
def done(ctx, fin_taskid):
    ...
    if str(fin_taskid) in ctx.obj['TASKS'].keys():
        ...
        if ctx.obj['TASKS']:
            ...
            click.ech0('Error: no task with id ' + str(fin_taskid))
